...
const create = (sale) => database.transaction(() => {
  ...
  const inventoryControlEnabled = isInventoryControlEnabled();
  ...
  for (const item of sale.items) {
    ...
    if (inventoryControlEnabled && quantity > previousStock) throw new Error(`Not enough stock for ${item.name || upc}. Available: ${previousStock}`);
